# ===== fscore_cad core (entry renamed to _core_agent) =====
"""V31 three-day expiring late router with capacity confirmation."""
import base64
import copy
import json
import zlib
...
_AM_CONFIG = {'start_step': 456, 'reserve_early': 12, 'reserve_mid': 8, 'reserve_late': 6, 'reserve_tail': 3, 'shed_soft': 72, 'shed_hard': 88, 'shed_reserve_cut': 7, 'price_gate': 0.66, 'pressure_trigger': 2.0, 'pressure_gate_cut': 0.18, 'capacity_trigger': 0, 'capacity_gate_cut': 0.08, 'tranche': 4, 'pressure_tranche': 4, 'shed_tranche': 4, 'tail_tranche': 5, 'demand_reserve': 2, 'demand_reserve_cap': 14, 'demand_gate': 0.045, 'demand_gate_cap': 0.24, 'max_extra_per_item': 18, 'mirror_latch_step': 289, 'mirror_composition_distance': 2, 'mirror_money_distance': 250, 'mirror_max_extra_per_item': 0, 'mirror_urgent_max_extra_per_item': 6, 'late_first_step': 432, 'late_second_step': 576, 'late_cash_gap': 1000, 'late_protect_price_index': 0.78, 'late_convert_price_index': 0.82, 'late_reserve_shift': 2, 'late_gate_shift': 0.04, 'late_tranche_shift': 2, 'late_epoch_turns': 72}
_AM_STATE = {
    0: {"last_step": -1, "last_inventory": {}, "last_sold": {}, "last_shops": (), "pressure": {}, "added": {}, "near_mirror": None, "late_mode": "balanced", "mode_epoch": -1},
    1: {"last_step": -1, "last_inventory": {}, "last_sold": {}, "last_shops": (), "pressure": {}, "added": {}, "near_mirror": None, "late_mode": "balanced", "mode_epoch": -1},
}
...
def _am_price_index(prices):
    ratios = []
    for item in _AM_ITEMS:
        value = _get(prices, item, None)
        price = _AM_BASE_PRICE[item] if value is None else max(0.0, float(value or 0))
        ratios.append(price / _AM_BASE_PRICE[item])
    return sum(ratios) / len(ratios)


def _am_capacity_gap(obs):
    return sum(_am_public_capacity(obs, item) for item in _AM_ITEMS)


def _am_update_late_mode(obs, state, step, total_shed, prices):
    ...
    if epoch < 0 or int(state.get("mode_epoch", -1)) == epoch:
        return str(state.get("late_mode", "balanced"))

    mode = "balanced"
    if not bool(state.get("near_mirror", False)) and total_shed < _AM_CONFIG["shed_hard"]:
        cash_gap = _am_cash_gap(obs)
        price_index = _am_price_index(prices)
        capacity_gap = _am_capacity_gap(obs)
        gap = float(_AM_CONFIG["late_cash_gap"])
        if (
            cash_gap >= gap
            and price_index <= float(_AM_CONFIG["late_protect_price_index"])
            and total_shed < _AM_CONFIG["shed_soft"]
            and capacity_gap <= 0
        ):
            mode = "protect"
        elif (
...
def _adaptive_market(action, obs, step):
    ...
    late_mode = _am_update_late_mode(obs, state, step, total_shed, prices)
    ...
    if step >= _AM_CONFIG["start_step"]:
        for item in _AM_ITEMS:
            ...
            if not urgent and late_mode == "protect" and step < 648:
                reserve += int(_AM_CONFIG["late_reserve_shift"])
            elif not urgent and late_mode == "convert":
                reserve = max(0, reserve - int(_AM_CONFIG["late_reserve_shift"]))
            ...
            if bool(state.get("near_mirror", False)):
                maximum_extra = int(
                    _AM_CONFIG[
                        "mirror_urgent_max_extra_per_item"
                        if urgent
                        else "mirror_max_extra_per_item"
                    ]
                )
            else:
                maximum_extra = int(_AM_CONFIG["max_extra_per_item"])
            ...
            if not urgent and late_mode == "convert":
...
_FUND_SEED_COST = {"WHEAT": 10, "CARROT": 20, "TOMATO": 50, "STRAWBERRY": 100, "MELON": 80}
_FUND_ANIMAL_COST = {"GOOSE": 300, "COW": 400, "SHEEP": 500}
_FUND_LAND_COST = (1000, 2000, 4000)


def _fund_hire_cost(already_hired):
    a, b = 1, 1
    for _ in range(max(0, int(already_hired))):
        a, b = b, a + b
    return a


def _fund_current_action(action, obs, step):
    if step < int(_AM_CONFIG["late_first_step"]):
        return action
    orders = [list(order) for order in (action.get("market") or [])]
    if not any(len(order) >= 3 and order[0] == "SELL" for order in orders):
        return action

    farm = _farm(obs, _seat(obs))
    private = _get(obs, "private", {}) or {}
    available = {
        str(item): max(0, int(value or 0))
        for item, value in dict(_get(private, "shed", {}) or {}).items()
    }
    ...
    for index, order in enumerate(orders):
        ...
        if kind == "SELL" and len(order) >= 3:
            sold = min(quantity, available.get(item, 0))
            available[item] = available.get(item, 0) - sold
            # A discount keeps the check conservative under a falling price curve.
            cash += sold * max(1.0, float(_get(prices, item, 1) or 1)) * 0.75
            continue

        cost = 0.0
        if kind == "HIRE":
            cost = float(_fund_hire_cost(hires))
        elif kind == "BUY_LAND" and 1 <= quadrants <= 3:
            cost = float(_FUND_LAND_COST[quadrants - 1])
        elif kind == "BUY_SEED":
            cost = float(_FUND_SEED_COST.get(item, 0) * quantity)
        elif kind == "BUY_PRODUCT":
            cost = float(max(1, int(_get(prices, item, 1) or 1)) * quantity)
        elif kind == "BUY_ANIMAL":
            cost = float(_FUND_ANIMAL_COST.get(item, 0) * quantity)

        if cost > cash and any(
            len(later) >= 3 and later[0] == "SELL"
            for later in orders[index + 1:]
        ):
            needs_reorder = True
            break
        if cost > cash:
    ...
    sales = [order for order in orders if len(order) >= 3 and order[0] == "SELL"]
    others = [order for order in orders if not (len(order) >= 3 and order[0] == "SELL")]
    result["market"] = (sales + others)[:10]
    return result
...
def _core_agent(obs):
    try:
        ...
        action = _fund_current_action(action, obs, step)
        return _align_hands(action, obs)
    except Exception:
        farm = _farm(obs, _seat(obs))
        return {
...
# ===== v9 market-layer improvements (final agent) =====
"""fscore_cad + 4 duel-validated market-layer improvements (consolidated).

Wraps the proven fscore_cad champion (fscore_core.py) and applies, in order:
  1. contested-value SELL-slot ordering  (sell the good that crashes hardest first)
  2. protect feed-wheat                   (never sell wheat below 2x current herd)
  3. full terminal liquidation at step 718 (unsold scores $0, so dump everything)
  4. aggressive endgame premium dump (day>=28)

All are market-order-only, so the position-locked tape never desyncs. Each beat
the current champion head-to-head across seeds and both seats (duel.py):
  vs baseline fscore_cad: +5,740 avg margin, 12/12.
"""
import math
...
def _herd(obs):
    ...
    return sum(1 for row in _get(farm, "tiles", []) or []
               for t in (row or []) if isinstance(t, dict) and t.get("animal"))


def _reorder_sells(obs, act):
    orders = [list(o) for o in (act.get("market") or [])]
    pos = [i for i, o in enumerate(orders)
    ...
    def key(o):
        it = str(o[1]); q = max(1, int(o[2] or 1)); cur = int(_get(inv, it, I0) or I0)
        return (_contested(it, cur, q), _price(it, cur), it)

    orig = [orders[i] for i in pos]
    ranked = sorted(orig, key=key, reverse=True)
    if orig != ranked:
        for p, o in zip(pos, ranked):
            orders[p] = o
        act = dict(act); act["market"] = orders
    return act


def _protect_feed(obs, act):
    herd = _herd(obs)
    shed = int((_get(_get(obs, "private", {}) or {}, "shed", {}) or {}).get("WHEAT", 0) or 0)
    buf = 2 * max(1, herd)
    mkt = []
...
def _liquidate(obs, act, items, cap10=True):
    shed = dict((_get(obs, "private", {}) or {}).get("shed", {}) or {})
    inv = _get(_get(obs, "market", {}) or {}, "inventory", {}) or {}
    already = {}
    for o in act.get("market") or []:
    ...
    for it in items:
        ...
        if q <= 0:
            continue
        base_inv = int(_get(inv, it, I0) or I0)
        rev = sum(_price(it, base_inv + k) for k in range(q)) if it in PARAMS else q
    ...
    act = dict(act); act["market"] = mkt[:10] if cap10 else mkt
    return act
...
def agent(obs, config=None):
    try:
        ...
        if not isinstance(act, dict):
            return act
        act = _reorder_sells(obs, act)
        act = _protect_feed(obs, act)
        raw = _get(obs, "step", None)
        day = int(_get(obs, "day", 0) or 0)
        hour = int(_get(obs, "hour", 0) or 0)
        step = int(raw) if raw is not None else day * 24 + hour
        if step == 718:
            act = _liquidate(obs, act, PRODUCTS)
        if day >= 28:
            act = _liquidate(obs, act, PREMIUM)
        return act
    except Exception:
        try:
